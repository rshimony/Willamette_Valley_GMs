...
for i in range(nb):
    ...
    if i == 0:
        ...
        ni0 = np.int32(len(block1_vp.utme))   # no of grid points along y axis
        nj0 = np.int32(len(block1_vp.utmn))   # no of grid points along x axis
        ...
        nc_blk = np.int32(5)
        ni_blk = np.int32(len(globals()['block'+str(i)+'_vp'].utme))
        nj_blk = np.int32(len(globals()['block'+str(i)+'_vp'].utmn))
...
with open(rfilename, 'ab') as f:
    (np.zeros([nj0,ni0]).astype(np.float32)).tofile(f)
...
for blk in range(nb-1):
    ...
    ni_blk = np.int32(len(vp_blk.utme))
    nj_blk = np.int32(len(vp_blk.utmn))
    hhi_blk = hhi[blk]
    ...
    with open(rfilename, 'ab') as f:
        for i in range(ni_blk):
            for j in range(nj_blk):
                ...
                vp = vp_blk.vp.values[i,j,:]
                vs = vs_blk.vs.values[i,j,:]
                ...
                if blk == 1 or blk == 2:
                    ...
                    vp_prev = vp_blk.vp.values
                    vs_prev = vs_blk.vs.values
                    ...
                    vp = np.dstack((vp_prev_base.T, vp))
                    vs = np.dstack((vs_prev_base.T, vs))
